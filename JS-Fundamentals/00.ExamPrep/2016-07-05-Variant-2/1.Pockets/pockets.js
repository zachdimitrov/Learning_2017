function solve(args) {
    let n = args[0].split(' ').map(Number);

    function isPeak(index, n) {
    ...
    for (let i = 0; i < n.length; i++) {
        // let temp = '';
        // if (isValley(i, n)) temp = 'Valley';
        // if (isPeak(i, n)) temp = 'Peak';
        // console.log(i + ' - ' + n[i] + ' - ' + temp);
        if (i === 0) {
            continue;
        }
        if (i === n.length - 1) {
        ...
        if (isValley(i, n) && isPeak(i - 1, n) && isPeak(i + 1, n)) {
            sum += n[i];
        }
        if (maxSum < sum) {
            maxSum = sum;
...
// tests
console.log('Results are - 5, 3, 2, 66, 1010');
